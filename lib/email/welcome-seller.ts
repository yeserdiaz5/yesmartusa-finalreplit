...
interface SendSellerReviewEmailParams {
  to: string
  sellerName: string
}
...
<body style="margin: 0; padding: 0; font-family: 'Arial', sans-serif; background-color: #f5f5f5;">
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color: #f5f5f5; padding: 40px 20px;">
    ...
    console.log("[v0] Welcome email sent successfully:", data)
...
export async function sendSellerReviewEmail({
  to,
  sellerName,
}: SendSellerReviewEmailParams) {
  const safeName = escapeHtml(sellerName)
  ...
  try {
    const { data, error } = await resend.emails.send({
      from: "YesmartUSA <no-reply@yesmartusa.com>",
      to: [to],
      subject: "Your YesmartUSA Seller Account is Being Reviewed",
      html: `
<!DOCTYPE html>
<html>
<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Account Under Review - YesmartUSA</title>
</head>
<body style="margin: 0; padding: 0; font-family: 'Arial', sans-serif; background-color: #f5f5f5;">
  <table width="100%" cellpadding="0" cellspacing="0" border="0" style="background-color: #f5f5f5; padding: 40px 20px;">
    <tr>
      <td align="center">
        <table width="600" cellpadding="0" cellspacing="0" border="0" style="background-color: #ffffff; border-radius: 8px; overflow: hidden; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
          ...
          <tr>
            <td style="padding: 40px 30px;">
              <h2 style="margin: 0 0 20px 0; color: #1f2937; font-size: 24px; font-weight: bold;">
                Thanks for Signing Up, ${safeName}!
              </h2>
              
              <p style="margin: 0 0 20px 0; color: #4b5563; font-size: 16px; line-height: 1.6;">
                Thank you for completing your seller account application with YesmartUSA. We have received your information and Stripe is now reviewing your account.
              </p>

              <!-- Info Box -->
              <div style="background-color: #fef3c7; border-left: 4px solid #f59e0b; padding: 20px; margin: 20px 0; border-radius: 4px;">
                <p style="margin: 0 0 12px 0; color: #92400e; font-weight: bold; font-size: 16px;">Account Review in Progress</p>
                <p style="margin: 0; color: #92400e; font-size: 14px;">This process typically takes 5-15 minutes in test mode, or up to 24 hours in production.</p>
              </div>
              ...
              <div style="background-color: #eff6ff; border-left: 4px solid #3b82f6; padding: 20px; margin: 20px 0; border-radius: 4px;">
                <p style="margin: 0; color: #1e40af; font-size: 14px; line-height: 1.6;">
                  <strong>Note:</strong> While your account is under review, you can explore your seller dashboard, but you will not be able to list products until the verification is complete.
                </p>
              </div>
              ...
              <p style="margin: 30px 0 0 0; color: #4b5563; font-size: 16px; line-height: 1.6;">
                Thank you for choosing YesmartUSA. We will notify you as soon as your account is ready!
              </p>
    ...
    console.log("[v0] Review email sent successfully:", data)
    return { success: true, data }
  } catch (error: any) {
    console.error("[v0] Error sending review email:", error)
